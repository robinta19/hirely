...
function CreateRoomForm() {
  ...
  const paramName = searchParams.get('interviewerName') || '';

  const [title, setTitle] = useState(paramTitle || 'Frontend Developer Interview');
  const [interviewerName, setInterviewerName] = useState(paramName || 'Nata');
  ...
  useEffect(() => {
    ...
    if (paramName) setInterviewerName(paramName);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.room.hostKey) {
        sessionStorage.setItem(`hirely_host_${data.room.id}`, data.room.hostKey);
      }
      ...
      <header className="relative z-50 border-b border-zinc-800 bg-[#050507] sticky top-0">
        <div className="max-w-4xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <Link href="/" className="flex items-center gap-2 group">
            <span className="font-geist font-extrabold text-lg tracking-wider text-white">
              Hirely
            </span>
          </Link>
        </div>
